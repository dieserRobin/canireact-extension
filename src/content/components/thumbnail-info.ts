...
export async function addThumbnailReactionInfo(
  thumbnail: HTMLElement,
  response: Guidelines | null
): Promise<void> {
  ...
  const existingReactionInfo = thumbnail.querySelector(
  ...
  if (response.rules || response.info_text) {
    ...
    reactionInfo.className = `reaction-thumbnail-info ${response.info_text ? "" : response.rules?.stream.stream_reactions_allowed ? "reaction-info-success" : response.rules?.stream.stream_reactions_generally_allowed ? "reaction-info-orange" : "reaction-info-error"}`;
    reactionInfo.innerText = response.info_text
      ? "ℹ️"
      : response.rules?.stream.stream_reactions_allowed
        ? `✔ ${await getLanguageString("short_stream_reactions_allowed")}`
        : response.rules?.stream.stream_reactions_generally_allowed
          ? `⏲ ${await getLanguageString("short_stream_reactions_soon_allowed")}`
          : `✘ ${await getLanguageString("short_stream_reactions_not_allowed")}`;
...
export async function removeAllThumbnailInfos(): Promise<void> {
  ...
  thumbnails.forEach((thumbnail) => {
    const reactionInfo = thumbnail.querySelector(
      ".reaction-thumbnail-info-container"
    );
    if (reactionInfo) {
      reactionInfo.remove();
